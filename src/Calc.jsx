...
function reducer(state, { type,payload }){
    switch(type){
        ...
        case ACTIONS.CHOOSE_OPERATION :
            if(state.currentOperand === undefined && state.prevOperand === undefined) return state;
        ...
        case ACTIONS.EVALUATE:
            if(state.currentOperand === undefined || state.prevOperand === undefined || state.operation === undefined) return state;
...
export default function Calc(){
    ...
    return(
        <div className=" border-gray-400 shadow-2xl h-4/6 w-80 bg-cyan-100 grid grid-rows-7 grid-cols-4 gap-1 bg-gradient-to-r from-gray-200 via-gray-400 to-gray-600">
            <div className=" p-5 text-white font-mono col-span-4 row-span-2 flex flex-col bg-gradient-to-tr from-gray-900 to-gray-600 bg-gradient-to-r">
                <div className="text-2xl">{prevOperand} {operation}</div>
                <div className=" text-4xl">{currentOperand}</div>
            </div>
            <div className="col-span-2 row-span-1 flex justify-center items-center font-medium text-xl text-gray-800 hover:bg-slate-400" onClick={() => dispatch({ type : ACTIONS.CLEAR})}>AC</div>
            ...
            <div className=" col-span-2 flex justify-center items-center text-2xl text-gray-900 hover:bg-slate-400" onClick={() => dispatch({ type: ACTIONS.EVALUATE})}>=</div>
        </div>
    )
}